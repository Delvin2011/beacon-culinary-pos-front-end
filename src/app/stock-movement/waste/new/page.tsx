...
export default function StockMovementNewWastePage() {
  ...
  const columnConfig = useMemo(
    () => ({
      reasonRequirement: "required" as const,
  ...
  const handleSubmit = async (payload: LineItemSheetSubmitPayload) => {
    ...
    if (!res.ok) {
      throw new Error(parseError(body, "Unable to submit waste request."));
    }

    const id = body && typeof body === "object" ? (body as { id?: number }).id : undefined;
    toast({
      title: "Waste request submitted",
        ...
        <div className="rounded-2xl border bg-card p-6 shadow-sm">
          {loadingIngredients ? (
            ...
            <LineItemSheet
              ...
              submitLabel="Submit Waste Request"
